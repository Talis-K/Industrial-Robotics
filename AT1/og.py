...
class LabAT1():
    def do_it(self):
        ...
        # Create a ground
        # ground = Cuboid(scale=[3, 3, 0.01], pose=SE3(), color=[0.8, 0.8, 0.5, 0], collision=True)
        
        # Create a UR3 robot instance and store it as an instance attribute
        self.robot = UR3()
        self.robot.q = np.array([0, 0, 0, -pi/2, -pi/2, 0])  # Set initial joint configuration
        self.robot.links[3].qlim = np.deg2rad([0, 180])  # Set joint limits for the third link        
        ...
        self.robot.tool = SE3(0, 0, tool_offset)
        ...
        for i in range(len(brick_pose)):  # for each brick
            ...
            T1 = self.robot.fkine(self.robot.q)  # pose of end effector
            q_robot = self.robot.ikine_LM(T1, q0=self.robot.q, joint_limits = True).q  # find joint angles from inverse kinematics
            print(f"Start point: \n{T1}")  # print starting pose of end effector

            q_brick = self.robot.ikine_LM(brick_pose[i] * SE3(0, 0, brick_offset), q0=self.robot.q).q  # find joint angles from inverse kinematics
            ...
            for q in q_matrix:  # for each set of joint angles in the trajectory
                if q2_joint_limits:  # if the joint angles are valid       
                    self.robot.q = q  # update robot joint angles                                                                                                                    
            ...
            print(f"End point: \n{self.robot.fkine(self.robot.q)}")  # print end pose of end effector         
            print("------------------------------------------------------------------------------------------------")  # print a separator line
            
            # End effector transform
            T1 = self.robot.fkine(self.robot.q)  # pose of end effector
            q_robot2 = self.robot.ikine_LM(T1, q0=self.robot.q, joint_limits = True).q 
            print(f"Start point: \n{T1}")  # print starting pose of end effector

            # Wall transform
            q_wall = self.robot.ikine_LM(wall_pose[i], q0=np.zeros(6)).q  # find joint angles from inverse kinematics                    
            print(f"Wall: \n{wall_pose[i]}")  # print pose of wall    

            # Confirm joint angles are within limits and generate trajectory
            q_goal_joint_limits = self.check_joint_limits(q_wall, self.robot)  # check if joint angles are within limits    
            q_matrix = rtb.jtraj(q_robot2, q_wall, steps).q  # generate a smooth joint trajectory  

            # Animate
            for q in q_matrix:  # for each set of joint angles in the trajectory
                if q_goal_joint_limits:  # if the joint angles are valid 
                    self.robot.q = q  # update robot joint angles  
                    bricks[i].T = self.robot.fkine(self.robot.q)  # update brick position
                    env.step(0.05)  # step the environment
                    time.sleep(0.02)  # wait a short time to make the animation visible
                else:
